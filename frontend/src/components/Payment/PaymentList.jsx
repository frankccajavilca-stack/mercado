  ...
  return (
    <div className="card" style={{marginTop:12}}>
      ...
      <table className="payments-table" aria-label="Pagos">
        <thead>
          <tr>
            <th>ID</th><th>Cliente</th><th>Monto</th><th>Estado</th>
          </tr>
        </thead>
        <tbody>
          {payments.map(p=>(
            <tr key={p.payment_id || p.id}>
              ...
              <td>${p.amount}</td>
              <td>
                <span className={ p.status==='aprobado' ? 'status-pill status-approved' : p.status==='pendiente' ? 'status-pill status-pending' : 'status-pill status-rejected'}>
                  {p.status}
                </span>
              </td>
            </tr>
